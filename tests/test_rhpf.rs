...
fn test_rhpf_attenuates_lows() {
    ...
    println!(
        "RHPF - Low energy: {}, High energy: {}, Ratio: {}",
...
fn test_rhpf_resonance_peak() {
    ...
    let code = r#"
        tempo: 0.5
        out $ white_noise # rhpf 1000 10.0
    "#;
...
fn test_rhpf_no_dc_offset() {
    ...
    assert!(
        mean.abs() < 0.02,
        "RHPF should have no DC offset, got {}",
        mean
    );
...
fn test_rhpf_rhythmic_filtering() {
    ...
    let code = r#"
        tempo: 0.5
        ~sweep $ line 100 2000
        out $ white_noise # rhpf ~sweep 5.0
    "#;
    ...
    assert!(
        rms > 0.1,
        "RHPF rhythmic filtering should work, RMS: {}",
        rms
    );
...
fn test_rhpf_cascade() {
    ...
    let code = r#"
        tempo: 0.5
        out $ saw 110 # rhpf 800 2.0 # rhpf 1000 2.0
    "#;
...
fn test_rhpf_rlpf_combo() {
    ...
    let code = r#"
        tempo: 0.5
        out $ white_noise # rhpf 500 1.0 # rlpf 2000 1.0
    "#;
...
fn test_rhpf_very_low_cutoff() {
    ...
    assert!(
        rms > 0.2,
        "RHPF should work at very low cutoff, RMS: {}",
        rms
    );
...
fn test_rhpf_very_high_cutoff() {
    let code = r#"
        tempo: 0.5
        out $ white_noise # rhpf 15000 2.0
    "#;
    ...
    assert!(
        rms > 0.01,
        "RHPF should work at very high cutoff, RMS: {}",
        rms
    );
