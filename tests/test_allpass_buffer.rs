...
fn test_allpass_buffer_vs_sample() {
    ...
    let sine_id = graph.add_oscillator(440.0, phonon::unified_graph::Waveform::Sine);

    // Create allpass with coefficient 0.7
    let allpass_id = graph.add_allpass_node(
        Signal::Node(sine_id),
        Signal::Value(0.7)
    );
...
fn test_allpass_buffer_cascade() {
    ...
    let ap1 = graph.add_allpass_node(Signal::Node(sine_id), Signal::Value(0.3));
    let ap2 = graph.add_allpass_node(Signal::Node(ap1), Signal::Value(0.5));
    let ap3 = graph.add_allpass_node(Signal::Node(ap2), Signal::Value(0.7));
...
fn test_allpass_buffer_modulated_coefficient() {
    ...
    let lfo_id = graph.add_oscillator(2.0, phonon::unified_graph::Waveform::Sine);
...
fn test_allpass_buffer_for_reverb() {
    ...
    let sine_id = graph.add_oscillator(220.0, phonon::unified_graph::Waveform::Sine);

    // Classic Schroeder allpass cascade (simplified)
    let ap1 = graph.add_allpass_node(Signal::Node(sine_id), Signal::Value(0.131));
    let ap2 = graph.add_allpass_node(Signal::Node(ap1), Signal::Value(0.359));
    let ap3 = graph.add_allpass_node(Signal::Node(ap2), Signal::Value(0.677));
    let ap4 = graph.add_allpass_node(Signal::Node(ap3), Signal::Value(0.773));
