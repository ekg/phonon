...
fn test_trim_is_alias_for_zoom() {
    ...
    let trimmed = pattern.clone().trim(0.0, 0.5).query(&state);
    let zoomed = pattern.clone().zoom(0.0, 0.5).query(&state);
...
fn test_humanize_is_alias_for_shuffle() {
    ...
    let humanized = pattern.clone().humanize(0.1, 0.0).query(&state);
