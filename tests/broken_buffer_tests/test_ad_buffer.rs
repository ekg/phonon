...
fn test_ad_basic_envelope_shape() {
    ...
    let ad_id = graph.add_ad_node(
        Signal::Value(0.01),  // 10ms attack
        Signal::Value(0.1),   // 100ms decay
    );
...
fn test_ad_different_attack_times() {
    ...
    // Fast attack
    let fast_id = graph.add_ad_node(
        Signal::Value(0.001),  // 1ms
        Signal::Value(0.05),
    );

    // Slow attack
    let slow_id = graph.add_ad_node(
...
fn test_ad_simpler_than_adsr() {
    ...
    let adsr_id = graph.add_adsr_node(
        Signal::Value(0.01),   // attack
        Signal::Value(0.05),   // decay
        Signal::Value(0.0),    // sustain = 0
        Signal::Value(0.001),  // release (minimal)
    );
...
fn test_ad_typical_percussive() {
    ...
    let ad_id = graph.add_ad_node(
        Signal::Value(0.001),  // 1ms attack (instant)
        Signal::Value(0.15),   // 150ms decay (natural decay)
    );
