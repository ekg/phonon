...
fn test_alternation_generates_different_cycles() {
    ...
    for cycle in 0..4 {
        ...
        if end <= audio.data.len() {
            ...
            let cycle_rms: f32 = (cycle_samples.iter()
                .map(|x| x * x)
                .sum::<f32>() / cycle_samples.len() as f32)
                .sqrt();
            
            println!("\nCycle {}: {} peaks detected, RMS: {:.3}", cycle, peaks.len(), cycle_rms);
            
            // For debugging, print first few samples to see if there's any signal
            if cycle_samples.len() > 100 {
                let first_100_max = cycle_samples[..100].iter()
                    .map(|x| x.abs())
...
fn test_verify_sample_pattern_timing() {
    ...
    for cycle in 0..4 {
        ...
        assert_eq!(events.len(), 1, "Cycle {} should have exactly 1 event", cycle);
        
        let expected = if cycle % 2 == 0 { "bd" } else { "sn" };
        assert_eq!(events[0].value, expected, "Cycle {} should be '{}'", cycle, expected);
