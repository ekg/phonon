...
fn test_deeply_nested_groups() {
    ...
    assert_eq!(
        haps.len(),
        1,
        "Deeply nested groups should still produce one event"
    );
...
fn test_complex_nesting_with_operators() {
    ...
    assert_eq!(
        haps.len(),
        8,
        "Complex nested pattern with operators should produce 8 events"
    );
...
fn test_alternation_in_polyrhythm() {
    ...
    for cycle in 0..2 {
        ...
        assert_eq!(
            haps.len(),
            5,
            "Polyrhythm with alternation should have 5 events"
        );
...
#[test]
#[ignore = "UNIMPLEMENTED: mini-notation edge case"]
...
fn test_random_choice_consistency() {
    ...
    assert_eq!(
        haps.len(),
        1,
        "Random choice should produce exactly one event"
    );
...
fn test_euclidean_edge_cases() {
    ...
    assert_eq!(
        haps.len(),
        0,
        "Euclidean with 0 pulses should produce no events"
    );
    ...
    assert_eq!(
        haps.len(),
        1,
        "Euclidean with 1 pulse should produce 1 event"
    );
}
...
fn test_division_edge_cases() {
    ...
    assert_eq!(
        all_haps.len(),
        1,
        "bd/100 should produce 1 event in 100 cycles"
    );
}
...
fn test_pattern_with_only_operators() {
    ...
    assert_eq!(
        haps.len(),
        0,
        "Pattern with only silence should produce no events"
    );
}
...
fn test_very_long_pattern() {
    ...
    assert_eq!(
        haps.len(),
        100,
        "Very long pattern should produce 100 events"
    );
    ...
    for (i, hap) in haps.iter().enumerate() {
        ...
        assert!(
            (actual_begin - expected_begin).abs() < 0.0001,
            "Begin time for event {} should be close to {}",
            i,
            expected_begin
        );
        assert!(
            (actual_end - expected_end).abs() < 0.0001,
            "End time for event {} should be close to {}",
            i,
            expected_end
        );
    }
