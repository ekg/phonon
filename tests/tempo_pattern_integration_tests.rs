...
fn test_pattern_transformation_fast() {
    ...
    env.eval(code)
        .expect("Failed to parse with fast transformation");
}
...
fn test_pattern_transformation_chain() {
    ...
    env.eval(code)
        .expect("Failed to parse transformation chain");
}
...
fn test_pattern_every_transformation() {
    ...
    env.eval(code)
        .expect("Failed to parse every transformation");
}
...
fn test_bpm_conversion_values() {
    ...
    for (bpm, expected_cps) in test_cases {
        ...
        env.eval(&code).expect("Failed to parse");
        assert!(
            (env.cps - expected_cps).abs() < 0.001,
            "BPM {} should give CPS {}, got {}",
            bpm,
            expected_cps,
            env.cps
        );
...
fn test_pattern_parser_integration() {
    ...
    for pattern in test_cases {
        ...
        parser
            .parse()
            .expect(&format!("Failed to parse: {}", pattern));
    }
}
