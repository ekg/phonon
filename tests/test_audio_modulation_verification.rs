...
fn detect_modulation(samples: &[f32], window_size: usize) -> bool {
    ...
    let variance: f32 = window_brightnesses.iter()
        .map(|b| (b - mean).powi(2))
        .sum::<f32>() / window_brightnesses.len() as f32;

    let std_dev = variance.sqrt();
    let coefficient_of_variation = std_dev / mean;

    println!("Mean brightness: {:.4}, Std Dev: {:.4}, CV: {:.4}",
...
fn test_filter_pattern_actually_modulates() {
    ...
    write_wav("/tmp/filter_pattern_test.wav", &samples, sample_rate)
    ...
    assert!(detect_modulation(&samples, window_size),
...
fn test_oscillator_frequency_pattern_modulates() {
    ...
    for i in 0..num_samples {
        ...
        if current_window.len() >= window_size {
            ...
            println!("Window {} freq estimate: {:.1} Hz",
                     zero_crossings_per_window.len(), freq);

            current_window.clear();
        }
    }

    write_wav("/tmp/freq_pattern_test.wav", &samples, sample_rate)
        .expect("Failed to write WAV");
    ...
    for &freq in &zero_crossings_per_window {
        if (freq - 110.0).abs() < 20.0 { found_110 = true; }
        if (freq - 220.0).abs() < 40.0 { found_220 = true; }
        if (freq - 440.0).abs() < 80.0 { found_440 = true; }
    }

    assert!(found_110 && found_220 && found_440,
            "Should find all three frequencies (110, 220, 440 Hz) in the pattern. \
             Found 110: {}, 220: {}, 440: {}", found_110, found_220, found_440);
}
...
fn test_pattern_timing_verification() {
    ...
    for i in 0..num_samples {
        ...
        if (sample - last_value).abs() > 0.01 {
            ...
            println!("Value change at sample {}: {:.2} -> {:.2}", i, last_value, sample);
            last_value = sample;
        }
    }
    ...
    if value_changes.len() >= 2 {
        ...
        for i in 1..value_changes.len() {
            ...
            assert!(error < 0.1,
                    ...
                    expected_interval as usize, interval);
        }
    }
}