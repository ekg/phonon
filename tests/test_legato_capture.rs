...
fn test_staccato_notes() {
    ...
    let legato_values: Vec<f32> = recorded
        .legato
        .split_whitespace()
    ...
    for (i, &legato) in legato_values.iter().enumerate() {
        assert!(
            legato < 0.2,
            "Staccato note {} should have low legato (< 0.2), got {}",
            i,
            legato
        );
...
fn test_legato_notes() {
    ...
    for (i, &legato) in legato_values.iter().enumerate() {
        assert!(
            legato > 0.8,
            "Legato note {} should have high legato (> 0.8), got {}",
            i,
            legato
        );
...
fn test_legato_pattern_alignment_with_rests() {
    ...
    assert_eq!(
        velocity_parts.len(),
        3,
        "Should have 3 velocity entries (including rest)"
    );
    ...
    assert_eq!(
        legato_parts.len(),
        3,
        "Should have 3 legato entries (including rest)"
    );
    ...
    assert!(
        legato_c4 > 0.7,
        "C4 should be legato (> 0.7), got {}",
        legato_c4
    );
    assert!(
        legato_d4 < 0.3,
        "D4 should be staccato (< 0.3), got {}",
        legato_d4
    );
...
fn test_legato_clamping() {
    ...
    assert_eq!(
        legato_values[0], 1.0,
        "Legato should be clamped to 1.0 for overlong note"
    );
