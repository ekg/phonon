...
mod timing_test {
    ...
    use phonon::pattern::{State, Fraction, TimeSpan};
    ...
    fn test_euclid_event_spacing() {
        ...
        for (i, event) in events.iter().enumerate() {
            ...
            let end = event.whole.as_ref().map(|w| w.end.to_float()).unwrap_or(0.0);
            let duration = end - start;
            println!("  Event {}: at {:.3}-{:.3} (duration: {:.3} cycles = {:.3}s)",
                i, start, end, duration, duration * 0.5);
        }
    }
}
