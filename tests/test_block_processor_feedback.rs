...
fn test_feedback_processes_without_crash() {
    ...
    let mut processor =
        BlockProcessor::new(nodes, 1, 512).expect("Should create processor with feedback");
    ...
    for _ in 0..10 {
        processor
            .process_block(&mut output, &context)
...
fn test_complex_feedback_network() {
    ...
    assert!(
        result.is_ok(),
        "Complex feedback networks should be allowed"
    );
