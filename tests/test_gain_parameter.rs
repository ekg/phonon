...
"#;
    ...
    assert!(rms > 0.01, "Normal gain should produce audible audio, got RMS={}", rms);
    assert!(peak > 0.3, "Normal gain should have significant peak, got peak={}", peak);
}
...
"#;
    ...
    assert!(double_rms > normal_rms * 1.5,
        "Double gain ({:.3}) should be ~2x normal gain ({:.3}), ratio: {:.2}",
        double_rms, normal_rms, double_rms / normal_rms);
}
...
"#;
    ...
    assert!(half_rms < normal_rms * 0.7,
        "Half gain ({:.3}) should be ~0.5x normal gain ({:.3}), ratio: {:.2}",
        half_rms, normal_rms, half_rms / normal_rms);

    assert!(half_rms > normal_rms * 0.3,
...
"#;
    ...
    assert!(quiet_rms < normal_rms * 0.2,
        "Quiet gain ({:.3}) should be much less than normal gain ({:.3}), ratio: {:.2}",
        quiet_rms, normal_rms, quiet_rms / normal_rms);
}
...
"#;
    ...
    assert!(rms > 0.01, "Pattern-based gain should produce audio, got RMS={}", rms);
    assert!(peak > 0.3, "Pattern-based gain should have significant peak, got peak={}", peak);

    // The highest gain (2.0) should create a significant peak
    assert!(peak > 0.8, "With gain up to 2.0, peak should be substantial, got peak={}", peak);
}
