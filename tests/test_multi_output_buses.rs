...
fn test_single_output_o1() {
    ...
    let mut graph = compile_to_audio_nodes(code, 44100.0).expect("Should compile out $");
...
fn test_two_outputs_o1_o2() {
    ...
    let mut graph = compile_to_audio_nodes(code, 44100.0).expect("Should compile out $ and o2:");
...
fn test_three_outputs_mixed() {
    ...
    let mut graph = compile_to_audio_nodes(code, 44100.0).expect("Should compile out $, o2:, o3:");
    ...
    for sample in &audio {
        assert!(
            (sample - 0.6).abs() < 0.001,
            "Expected 0.6 (0.1 + 0.2 + 0.3), got {}",
            sample
        );
    }
}
...
fn test_output_with_samples() {
    ...
    let mut graph = compile_to_audio_nodes(code, 44100.0).expect("Should compile sample outputs");
    ...
    assert!(
        rms > 0.01,
        "RMS should be > 0.01 (not silence), got {}",
        rms
    );
...
fn test_output_with_synthesis() {
    ...
    let mut graph =
        compile_to_audio_nodes(code, 44100.0).expect("Should compile synthesis outputs");
    ...
    assert!(
        rms > 0.9,
        "RMS should be > 0.9 (two sine waves), got {}",
        rms
    );
}
...
fn test_explicit_out_overrides_numbered() {
    ...
    let mut graph =
        compile_to_audio_nodes(code, 44100.0).expect("Should compile with explicit out:");
    ...
    for sample in &audio {
        assert!(
            ...
            "Expected 1.0 (out $ overrides), got {}",
            sample
        );
    }
}
...
fn test_bus_references_in_outputs() {
    ...
    let mut graph =
        compile_to_audio_nodes(code, 44100.0).expect("Should compile with bus references");
...
fn test_complex_multi_output() {
    ...
    let mut graph =
        compile_to_audio_nodes(code, 44100.0).expect("Should compile complex multi-output");
