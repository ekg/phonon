/// Direct fundsp square_hz API tests
///
/// Level 1: Verify fundsp API works correctly before wrapping
///
/// These tests ensure we understand fundsp's square_hz behavior:
/// - Takes 1 parameter (frequency)
/// - Takes 0 audio inputs (generator)
/// - Returns 1 mono output
/// - Bandlimited square waveform (only odd harmonics)
...
fn test_fundsp_square_hz_basic() {
    ...
    // Should have output
    assert!(sum > 0.0, "Square should produce output: {}", sum);
...
fn test_fundsp_square_hz_waveform_shape() {
    ...
    let mut unit = square_hz(100.0); // Low frequency to see shape
    unit.reset();
    unit.set_sample_rate(44100.0);
    ...
    println!("Square waveform range: {:.3} to {:.3}", min_val, max_val);
}
...
fn test_fundsp_square_hz_dc_centered() {
    ...
    assert!(
        average.abs() < 0.01,
        "Square should be DC-centered, average: {}",
        average
    );
