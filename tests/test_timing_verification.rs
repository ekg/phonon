...
fn test_synth_timing_is_even() {
    ...
    if onsets.len() >= 4 {
        ...
        for i in 0..3 {
            ...
            println!(
                "Spacing {}-{}: {} samples (expected {:.0}, error {:.1}%)",
                i,
                i + 1,
                spacing,
                expected_spacing,
...
fn test_timing_with_rests() {
    ...
    if onsets.len() >= 2 {
        ...
        assert!(
            (second_pos - 0.5).abs() < 0.01,
            "Second onset should be near 0.5s"
        );
    }
...
fn test_no_compression_at_end() {
    ...
    for quarter in 0..4 {
        ...
        let rms: f32 =
            quarter_data.iter().map(|x| x * x).sum::<f32>().sqrt() / (quarter_size as f32).sqrt();
    ...
    let last_rms: f32 = last_10_percent.iter().map(|x| x * x).sum::<f32>().sqrt()
        / (last_10_percent.len() as f32).sqrt();
    ...
    assert!(
        last_rms > 0.0001,
        "Should have audio decay at end, not silence"
    );
