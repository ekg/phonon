...
fn test_alternation_with_frequencies() {
    ...
    for cycle in 0..num_cycles {
        ...
        assert_eq!(events[0].value, expected, 
            "Cycle {} should have {}", cycle, expected);
    }
}
...
fn test_alternation_in_euclidean_patterns() {
    ...
    for cycle in 0..4 {
        ...
        println!("Cycle {}: {} events (excluding rests)", cycle, non_rest_events.len());
        
        // Check pulse count alternation (3, 4, 3, 4)
        let expected_pulses = if cycle % 2 == 0 { 3 } else { 4 };
        
        assert_eq!(non_rest_events.len(), expected_pulses,
            "Cycle {} should have {} pulses", cycle, expected_pulses);
...
fn test_simple_pattern_rendering() {
    ...
    if samples.data.len() >= window_size {
        ...
        assert!((freq - 440.0).abs() < 50.0, "Should detect ~440Hz, got {}", freq);
    }
}