...
fn test_latency_calculation() {
    ...
    assert!((latency_64 - 1.45).abs() < 0.01, "64 samples should be ~1.45ms");

    // 128 samples should be ~2.90ms
    let latency_128 = (128.0 / sample_rate) * 1000.0;
    assert!((latency_128 - 2.90).abs() < 0.01, "128 samples should be ~2.90ms");

    // 256 samples should be ~5.80ms
    let latency_256 = (256.0 / sample_rate) * 1000.0;
    assert!((latency_256 - 5.80).abs() < 0.01, "256 samples should be ~5.80ms");

    // 512 samples should be ~11.61ms
    let latency_512 = (512.0 / sample_rate) * 1000.0;
    assert!((latency_512 - 11.61).abs() < 0.01, "512 samples should be ~11.61ms");
}
