...
fn test_sidechain_compression_basic() {
    ...
    assert!(rms > 0.05, "Sidechain compression should produce audio, got RMS: {}", rms);
}
...
fn test_sidechain_compression_house_track() {
    ...
    assert!(rms > 0.1, "House track should have strong signal, got RMS: {}", rms);
}
...
fn test_sidechain_creates_pumping() {
    ...
    assert!(sidechained_rms < baseline_rms * 0.9,
        "Sidechain should reduce average level (baseline: {}, sidechained: {})",
        baseline_rms, sidechained_rms);
}
...
fn test_dub_echo_chain() {
    ...
    let code = r#"
        ...
        o1: ~source * 0.6 + ~echo1 * 0.3 + ~echo2 * 0.2
    "#;

    let buffer = render_dsl(code, 3.0);
    let rms = calculate_rms(&buffer);
    
    assert!(rms > 0.1, "Dub echo chain should produce audio, got RMS: {}", rms);
}
...
fn test_major_chord() {
    ...
    let code = r#"
        ...
        ~note2: sine 329.63
        ~note3: sine 392.00
        o1: (~note1 + ~note2 + ~note3) * 0.3
    "#;

    let buffer = render_dsl(code, 2.0);
    let rms = calculate_rms(&buffer);
    
    assert!(rms > 0.15, "Major chord should produce strong audio, got RMS: {}", rms);
}
...
fn test_minor_chord() {
    ...
    assert!(rms > 0.15, "Minor chord should produce strong audio, got RMS: {}", rms);
}
...
fn test_seventh_chord() {
    ...
    let code = r#"
        ...
        ~note3: sine 293.66
        ~note4: sine 349.23
        o1: (~note1 + ~note2 + ~note3 + ~note4) * 0.25
    "#;

    let buffer = render_dsl(code, 2.0);
    let rms = calculate_rms(&buffer);
    
    assert!(rms > 0.15, "7th chord should produce strong audio, got RMS: {}", rms);
}
...
fn test_chord_progression() {
    ...
    assert!(rms > 0.15, "Chord progression should produce audio, got RMS: {}", rms);
}
...
fn test_house_track_with_sidechain_and_chords() {
    ...
    let code = r#"
        ...
        o1: ~kick * 0.6 + ~ducked_bass * 0.4 + ~chord * 0.3
    "#;

    let buffer = render_dsl(code, 4.0);
    let rms = calculate_rms(&buffer);

    assert!(rms > 0.15, "Full house track should have strong signal, got RMS: {}", rms);
}
...
fn test_melodic_pattern_with_effects() {
    ...
    assert!(rms > 0.05, "Melodic pattern with effects should produce audio, got RMS: {}", rms);
}
...
fn test_vibrato_and_tremolo() {
    ...
    assert!(rms > 0.1, "Vibrato and tremolo should produce audio, got RMS: {}", rms);
}
