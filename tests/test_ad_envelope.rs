...
out $ ~env
    ...
    assert!(
        post_attack_avg > 0.9,
        "Post-attack should be near 1.0, got {}",
        post_attack_avg
    );
    ...
    let mid_avg: f32 = mid_samples.iter().sum::<f32>() / mid_samples.len() as f32;
    assert!(
        mid_avg > 0.4 && mid_avg < 0.7,
        "Mid-decay should be ~0.5, got {}",
        mid_avg
    );
    ...
    assert!(
        end_avg < 0.2,
        "End of decay should be near 0, got {}",
        end_avg
    );

    println!(
        "Decay phase - post-attack: {}, mid: {}, end: {}",
        post_attack_avg, mid_avg, end_avg
    );
}
...
out $ ~env
    ...
    assert!(
        silent_avg < 0.05,
        "After decay should be silent, got {}",
        silent_avg
    );
...
out $ ~tone ~* ~env ~* 0.5
    ...
    assert!(
        rms > 0.03,
        "AD percussive tone should be audible, got RMS: {}",
        rms
    );
...
out $ ~synth * 0.3
    ...
    assert!(
        rms > 0.03,
        "AD filter envelope should work, got RMS: {}",
        rms
    );
...
out $ ~env
    ...
    assert!(
        rms > 0.05,
        "AD with pattern-modulated attack should work, RMS: {}",
        rms
    );
...
out $ ~env
    ...
    for cycle in 0..4 {
        ...
        assert!(
            end_avg < 0.3,
            "Cycle {} end should be post-decay, got {}",
            cycle,
            end_avg
        );
...
out $ ~env
    ...
    assert!(
        rms > 0.001,
        "AD with very short times should work, got RMS: {}",
        rms
    );
