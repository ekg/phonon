...
fn test_query_pattern_at_boundaries() {
    ...
    let state0 = State {
        span: TimeSpan::new(Fraction::from_float(0.0), Fraction::from_float(0.001)),
        controls: HashMap::new(),
    };
    let events0 = pattern.query(&state0);
    println!(
        "Query at 0.0: {:?}",
        events0.iter().map(|e| &e.value).collect::<Vec<_>>()
    );

    // Query at position 0.5 (boundary between first and second)
    let state05 = State {
        span: TimeSpan::new(Fraction::from_float(0.5), Fraction::from_float(0.501)),
        controls: HashMap::new(),
    };
    let events05 = pattern.query(&state05);
    println!(
        "Query at 0.5: {:?}",
        events05.iter().map(|e| &e.value).collect::<Vec<_>>()
    );

    // Query at position 0.75 (second event)
    let state075 = State {
        span: TimeSpan::new(Fraction::from_float(0.75), Fraction::from_float(0.751)),
        controls: HashMap::new(),
    };
    let events075 = pattern.query(&state075);
    println!(
        "Query at 0.75: {:?}",
        events075.iter().map(|e| &e.value).collect::<Vec<_>>()
    );
    ...
    assert_eq!(
        events05[0].value, "2",
        "At boundary 0.5, should get second value"
    );
