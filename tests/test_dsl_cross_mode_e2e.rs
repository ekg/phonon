...
/// Test that reference DSL compiles via DslCompiler (used by all modes internally)
#[test]
fn test_dsl_compiler_direct() {
    let (remaining, statements) = parse_dsl(REFERENCE_DSL).expect("Should parse DSL");
...
fn test_dsl_audio_consistency_across_modes() {
    ...
    // Mode 1: Direct DslCompiler
    let (_, statements) = parse_dsl(REFERENCE_DSL).unwrap();
    let compiler = DslCompiler::new(sample_rate);
    let mut graph1 = compiler.compile(statements);
    ...
    println!("   DslCompiler RMS: {:.6}", rms1);
...
"#;
    ...
    let (_, statements) = parse_dsl(dsl).expect("Should parse");
    let compiler = DslCompiler::new(44100.0);
