...
fn test_set_cycle_jumps_to_position() {
    ...
    let position = graph.get_cycle_position();
    assert!((position - 5.5).abs() < 0.001, "Should be at cycle 5.5: got {}", position);

    // Jump to cycle 100
    graph.set_cycle(100.0);

    let position = graph.get_cycle_position();
    assert!((position - 100.0).abs() < 0.001, "Should be at cycle 100: got {}", position);

    // Jump backwards to cycle 2
    graph.set_cycle(2.0);

    let position = graph.get_cycle_position();
    assert!((position - 2.0).abs() < 0.001, "Should be at cycle 2: got {}", position);
}
...
fn test_set_cycle_in_dsl() {
    ...
    let position = graph.get_cycle_position();
    assert!((position - 42.7).abs() < 0.001, "Should be at cycle 42.7: got {}", position);
}
...
fn test_nudge_shifts_timing() {
    ...
    let position = graph.get_cycle_position();
    assert!((position - 10.25).abs() < 0.001, "Should be at 10.25 after nudge: got {}", position);

    // Nudge backward (advance) by 0.5 cycles
    graph.nudge(-0.5);

    let position = graph.get_cycle_position();
    assert!((position - 9.75).abs() < 0.001, "Should be at 9.75 after nudge: got {}", position);
    ...
    let position = graph.get_cycle_position();
    assert!((position - 10.0).abs() < 0.001, "Should be at 10.0 after nudges: got {}", position);
}
...
fn test_nudge_in_dsl() {
    ...
    let position = graph.get_cycle_position();
    assert!((position - 5.3).abs() < 0.001, "Should be at 5.3 (5.0 + 0.3 nudge): got {}", position);
}
...
fn test_nudge_negative_in_dsl() {
    ...
    let position = graph.get_cycle_position();
    assert!((position - 9.5).abs() < 0.001, "Should be at 9.5 (10.0 - 0.5 nudge): got {}", position);
}
...
fn test_multiple_commands_in_dsl() {
    ...
    let position = graph.get_cycle_position();
    assert!((position - 5.0).abs() < 0.001, "Final position should be 5.0: got {}", position);
}
...
fn test_timing_persists_through_wall_clock() {
    ...
    assert!((start - 10.0).abs() < 0.1, "Should start at ~10.0: got {}", start);
    ...
    assert!(after > 10.0, "Should have advanced from 10.0: got {}", after);
}
...
fn test_nudge_affects_wall_clock_mode() {
    ...
    assert!((after_nudge - before_nudge - 5.0).abs() < 0.1,
        "Nudge should shift by 5.0 cycles: before={}, after={}", before_nudge, after_nudge);
}
