...
fn test_waveguide_buffer_vs_sample() {
    ...
    // Sample-by-sample evaluation (use buffer evaluation on smaller buffers)
    let mut single_sample = vec![0.0; 1];
    for i in 0..buffer_size {
        graph2.eval_node_buffer(&wg2, &mut single_sample);
        sample_output[i] = single_sample[0];
    }
