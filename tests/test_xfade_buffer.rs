...
fn test_xfade_full_a() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Value(0.8),
        signal_b: Signal::Value(0.2),
        position: Signal::Value(0.0), // 100% A
    });
...
fn test_xfade_full_b() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Value(0.8),
        signal_b: Signal::Value(0.2),
        position: Signal::Value(1.0), // 100% B
    });
...
fn test_xfade_center() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Value(1.0),
        signal_b: Signal::Value(-1.0),
        position: Signal::Value(0.5), // 50/50 mix
    });
...
fn test_xfade_quarter_mix() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Value(1.0),
        signal_b: Signal::Value(0.0),
        position: Signal::Value(0.25),
    });
...
fn test_xfade_modulated_lfo() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        ...
        position: Signal::Node(lfo_offset),
    });
...
fn test_xfade_clamps_position() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Value(0.5),
        signal_b: Signal::Value(0.9),
        position: Signal::Value(2.0), // Should clamp to 1.0
    });
    ...
    let xfade_id2 = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Value(0.3),
        signal_b: Signal::Value(0.7),
        position: Signal::Value(-1.0), // Should clamp to 0.0
    });
...
fn test_xfade_positive_negative() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Value(0.6),
        signal_b: Signal::Value(-0.4),
        position: Signal::Value(0.5),
    });
...
fn test_xfade_square_waves() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Node(sq_a),
        signal_b: Signal::Node(sq_b),
        position: Signal::Value(0.3), // 70% A, 30% B
    });
...
fn test_xfade_sawtooth_triangle() {
    ...
    for mix in [0.0, 0.25, 0.5, 0.75, 1.0] {
        let xfade_id = graph.add_node(SignalNode::XFade {
            signal_a: Signal::Node(saw),
            signal_b: Signal::Node(tri),
            position: Signal::Value(mix),
        });
...
fn test_xfade_silence_to_sound() {
    ...
    let xfade_0 = graph.add_node(SignalNode::XFade {
        signal_a: silence.clone(),
        signal_b: Signal::Node(osc),
        position: Signal::Value(0.0),
    });
...
fn test_xfade_inverted_signals() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Node(osc),
        signal_b: Signal::Node(osc_inverted),
        position: Signal::Value(0.5),
    });
...
fn test_xfade_dynamic_position() {
    ...
    let xfade_id = graph.add_node(SignalNode::XFade {
        signal_a: Signal::Node(osc_low),
        signal_b: Signal::Node(osc_high),
        position: Signal::Node(pos_offset),
    });
