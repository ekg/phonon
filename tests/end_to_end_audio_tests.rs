...
fn test_sine_wave_generation() {
    ...
    assert!(buffer.peak() > 0.5, "Sine wave should have significant amplitude");
    ...
    assert!(!has_frequency(&buffer, 880.0, 0.1), "Should not contain 880Hz");
}
...
fn test_scalar_multiplication() {
    ...
    assert!((peak - 0.5).abs() < 0.1, "Peak should be ~0.5, got {}", peak);
}
...
fn test_addition_mixing() {
    ...
    assert!(buffer.peak() > 1.0, "Mixed signal should have higher amplitude");
}
...
fn test_subtraction() {
    ...
    assert!(buffer.peak() < 0.01, "Subtraction of identical signals should be near zero");
...
fn test_low_pass_filter() {
    ...
    assert!(low_freq_energy > high_freq_energy * 2.0, 
...
fn test_complex_arithmetic() {
    ...
    assert!(buffer.rms() > 0.01, "Should have signal, got RMS: {}", buffer.rms());
    ...
    buffer.write_wav("/tmp/phonon_test_complex_arithmetic.wav").unwrap();
}