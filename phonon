...
start_boson() {
    ...
    # Use PATTERN_FILE variable (can be absolute or relative)
    if [[ "$PATTERN_FILE" = /* ]]; then
        # Absolute path
        node boson-real.js watch "$PATTERN_FILE" $BPM_ARG &
    else
        # Relative path from boson directory
        node boson-real.js watch "../$(basename "$PATTERN_FILE")" $BPM_ARG &
    fi
    BOSON_PID=$!
    sleep 1
...
case "${1:-}" in
    start)
        ...
        # Check for optional pattern file argument
        if [ ! -z "${2:-}" ]; then
            if [ -f "$SCRIPT_DIR/${2}" ]; then
                PATTERN_FILE="$SCRIPT_DIR/${2}"
                echo -e "${GREEN}Using pattern file: ${2}${NC}"
            elif [ -f "${2}" ]; then
                PATTERN_FILE="${2}"
                echo -e "${GREEN}Using pattern file: ${2}${NC}"
            else
                echo -e "${RED}Pattern file not found: ${2}${NC}"
                exit 1
            fi
        fi
        ...
        echo -e "Pattern file: ${BLUE}$(basename "$PATTERN_FILE")${NC}"
    ...
    *)
        banner
        echo "Usage: $0 {start|test|edit|build|stop|clean} [pattern_file]"
        echo ""
        echo "  start [file] - Start with optional pattern file (default: patterns.phonon)"
        ...
        echo ""
        echo "Examples:"
        echo "  $0 start                    # Use default patterns.phonon"
        echo "  $0 start synth.phonon       # Use synth.phonon"
        echo "  $0 start test-pattern.phonon # Use test-pattern.phonon"
        echo ""
        exit 1
        ;;
esac