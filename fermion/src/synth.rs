...
impl SynthEngine {
    ...
    fn load_default_samples(&mut self) {
        ...
        self.samples.insert("kick".to_string(), self.samples["bd"].clone());
        
        // Snare: noise burst
        let snare = self.generate_snare();
        self.samples.insert("sd".to_string(), snare);
        self.samples.insert("snare".to_string(), self.samples["sd"].clone());
        
        // Hi-hat: short noise
        let hihat = self.generate_hihat();
        self.samples.insert("hh".to_string(), hihat);
        self.samples.insert("hihat".to_string(), self.samples["hh"].clone());
    ...
    pub fn load_sample(&mut self, name: &str, path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn load_dirt_sample(&mut self, name: &str, index: usize) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    ...
    pub fn play_sample(&mut self, path: &Path, sample_name: &str, index: usize, speed: f32) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("Playing sample: {}:{} at speed {}", sample_name, index, speed);
