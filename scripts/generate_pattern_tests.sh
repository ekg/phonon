#!/bin/bash
# Generate pattern parameter tests for common DSL functions
# This creates tests for nodes that can be called via DSL syntax

OUTPUT_FILE="tests/test_pattern_params_generated.rs"
...
fn test_bitcrush_pattern_bits() {
    let code = r#"
        tempo: 2.0
        o1: sine 440 # bitcrush (sine 1.0 * 4.0 + 8.0)
    "#;
    let buffer = render_dsl(code, 2.0);
...
echo "Generated $OUTPUT_FILE with pattern parameter tests"
chmod +x "$OUTPUT_FILE"
