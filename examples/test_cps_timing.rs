...
fn main() {
    ...
    for &cps in &[0.5, 1.0, 2.0] {
        ...
        for i in 1..audio.data.len() {
            if audio.data[i - 1].abs() < 0.001
                && audio.data[i].abs() > 0.001
                && audio.data[i + 1].abs() > 0.001
            {
        ...
        if beat_positions.len() >= expected_beats {
            ...
            for i in 0..expected_beats.min(8) {
                ...
                println!(
                    "    Beat {}: sample {} ({:.3}s), expected {:.3}s, error: {:.1}ms",
