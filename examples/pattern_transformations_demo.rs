...
fn main() {
    ...
    print_events("overlay (plays both)", pattern1.clone().overlay(pattern2.clone()));
    ...
    let chained = base.clone()
        .fast(2.0)
        .rev()
        .every(3, |p| p.slow(1.5));
    ...
    let categories = vec![
        ("Time", vec!["fast", "slow", "rev", "palindrome", "iter", "early", "late"]),
        ("Repetition", vec!["stutter", "echo", "ply"]),
        ("Combination", vec!["overlay", "stack", "append", "cat"]),
        ("Probability", vec!["degrade", "sometimes", "often", "rarely", "choose"]),
