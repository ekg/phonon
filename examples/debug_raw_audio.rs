...
fn main() {
    ...
    for beat in 0..4 {
        ...
        writeln!(file, "\n=== Beat {} (samples {}-{}) ===", beat, beat_start, beat_end).unwrap();
    ...
    for (idx, (start, end)) in audio_regions.iter().enumerate() {
        ...
        println!("Region {}: samples {}-{} ({:.4}s - {:.4}s), duration: {} samples ({:.4}s)",
        ...
        if (*start as i32 - expected_start as i32).abs() < 100 {
            println!("         ✓ Matches expected beat {} position", expected_beat);
        } else {
    ...
    for beat in 0..4 {
        ...
        println!("Beat {}: samples {}-{} ({:.3}s - {:.3}s)",
                 beat, expected_start, expected_end,
