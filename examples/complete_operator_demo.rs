...
fn demo_tonal_music() {
    ...
    let c_major = degrees.map(|s| s.parse::<f64>().unwrap_or(0.0))
        .scale("major", 60);
...
fn demo_signal_patterns() {
    ...
    // Envelopes
    let adsr = envelope(
        vec![(0.0, 0.0), (0.1, 1.0), (0.3, 0.7), (1.0, 0.0)],
        1.0
    );
...
fn demo_advanced_structures() {
    ...
    // Bite and chew
    let bitten = base.clone().bite(2, vec![
        Pattern::from_string("x y"),
        Pattern::from_string("1 2"),
    ]);
...
fn demo_midi_control() {
    ...
    let cc_pattern = values.map(|s| s.parse::<f64>().unwrap_or(0.0))
        .cc(7, 0); // Volume on channel 0
    ...
    // OSC patterns
    let osc = Pattern::from_string("play stop reset")
        .osc("/transport");
...
fn demo_pattern_analysis() {
    ...
    println!("  Stats: {} events, durations min={:.2} max={:.2} avg={:.2}", 
        count, min, max, avg);
