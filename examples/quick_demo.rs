...
fn main() {
    ...
    println!("  Slow 2x: {:?}", base.clone().slow(2.0).query_arc(0.0, 2.0));
...
fn print_pattern(pattern: &Pattern<String>) {
    ...
    println!("  Events: {:?}", haps.iter().map(|h| &h.value).collect::<Vec<_>>());
}
...
fn print_pattern_f64(pattern: &Pattern<f64>) {
    ...
    let haps = pattern.query(&state);
    println!("  Values: {:?}", haps.iter().map(|h| h.value).collect::<Vec<_>>());
}
