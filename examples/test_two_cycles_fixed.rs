...
fn main() {
    ...
    println!("Expected: {} (2 seconds at {}Hz)\n", (sample_rate * 2.0) as usize, sample_rate);
    ...
    println!("Found {} beat onsets (expecting 8 for 2 cycles):", beat_onsets.len());
    ...
    for beat in 0..8 {
        ...
        if beat < beat_onsets.len() {
            ...
            println!("{:4} | {:13.3}s | {:15} | {:13} | {:11.3}s | {:8.1}ms",
    ...
    if beat_onsets.len() >= 2 {
        ...
        for i in 1..beat_onsets.len() {
            ...
            println!("Beat {}-{} | {:7} | {:8.3}s | {:8.3}s | {:7.1}ms",
                     i-1, i, spacing, duration, expected_duration, deviation);
    ...
    if beat_onsets.len() >= 8 {
        ...
        let cycle1_spacings: Vec<usize> = (1..4).map(|i| beat_onsets[i] - beat_onsets[i-1]).collect();
        let cycle2_spacings: Vec<usize> = (5..8).map(|i| beat_onsets[i] - beat_onsets[i-1]).collect();
        
        println!("Cycle 1 spacings: {:?}", cycle1_spacings);
        println!("Cycle 2 spacings: {:?}", cycle2_spacings);
        
        let consistent = cycle1_spacings.iter()
    ...
    for (i, &onset) in beat_onsets.iter().enumerate() {
        ...
        println!("Beat {}: {} samples ({:.3}s), expected {:.3}s", 
