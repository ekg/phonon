...
fn main() {
    ...
    println!(
        "Event duration: {} samples ({:.3}s)",
        event_duration_samples,
        event_duration_samples as f32 / sample_rate
    );
    println!(
        "Attack: {} samples ({:.3}s)",
        attack_samples,
    ...
    for &sample_offset in &test_points {
        ...
        println!(
            "  Offset {:5} ({:.4}s): envelope = {:.6}",
            sample_offset, time, env
        );
        ...
        if sample_offset >= attack_samples && sample_offset < event_duration_samples {
            let decay_progress =
                (sample_offset - attack_samples) as f32 / decay_samples.max(1) as f32;
            println!(
                "    (decay_progress = {:.4}, exp({:.4}) = {:.6})",
                decay_progress,
                -5.0 * decay_progress,
                (-5.0 * decay_progress).exp()
            );
        }
    }
