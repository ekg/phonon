...
fn main() {
    ...
    for (bpm, description) in bpms {
        ...
        println!(
            "  In {} seconds: {:.1} cycles, {:.0} beats",
            duration, cycles, beats
        );
    }
    ...
    println!(
        "For {} BPM with {} beats per cycle:",
        target_bpm, beats_per_cycle
    );
    println!(
        "  CPS = {} / ({} * 60) = {:.3}",
        target_bpm, beats_per_cycle, target_cps
    );
