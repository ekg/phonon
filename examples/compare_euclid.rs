...
fn main() {
    ...
    let p1 =
        Pattern::<bool>::euclid(1, 8, 0).map(|b| if b { "x".to_string() } else { "-".to_string() });
    let p3 =
        Pattern::<bool>::euclid(3, 8, 0).map(|b| if b { "x".to_string() } else { "-".to_string() });
    ...
    if events1.len() == events3.len()
        && events1.iter().zip(events3.iter()).all(|(a, b)| {
    ...
    println!(
        "(2,5) - Khafif-e-ramal: {:?}",
        Pattern::<bool>::euclid(2, 5, 0)
    );
    println!(
        "(3,4) - Cumbia/Calypso: {:?}",
        Pattern::<bool>::euclid(3, 4, 0)
    );
    println!("(3,7) - Ruchenitza: {:?}", Pattern::<bool>::euclid(3, 7, 0));
    println!(
        "(3,8) - Cuban tresillo: {:?}",
        Pattern::<bool>::euclid(3, 8, 0)
    );
    println!("(4,9) - Aksak: {:?}", Pattern::<bool>::euclid(4, 9, 0));
    println!(
        "(5,8) - Cuban cinquillo: {:?}",
        Pattern::<bool>::euclid(5, 8, 0)
    );
    println!(
        "(5,16) - Bossa Nova: {:?}",
        Pattern::<bool>::euclid(5, 16, 0)
    );
    println!(
        "(7,12) - West African bell: {:?}",
        Pattern::<bool>::euclid(7, 12, 0)
    );
}
