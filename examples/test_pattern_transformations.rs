...
fn main() {
    ...
    for event in &events {
        println!("  {} at {:.3}-{:.3}", event.value, event.part.begin.to_float(), event.part.end.to_float());
    }
    ...
    for event in &jux_events {
        println!("  ({}, {}) at {:.3}-{:.3}", event.value.0, event.value.1, event.part.begin.to_float(), event.part.end.to_float());
    }
