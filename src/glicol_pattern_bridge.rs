...
impl PatternDspEngine {
    ...
    fn parse_pattern_to_control(&mut self, pattern_str: &str, control: &str) -> Result<(), String> {
        ...
        let value_pattern = pattern.fmap(|s| {
    ...
    fn parse_pattern_through_dsp(&mut self, pattern_str: &str, dsp_str: &str) -> Result<(), String> {
        let pattern = parse_mini_notation(pattern_str);
    ...
    pub fn add_modulation(&mut self,
        voice_name: &str,
        param_name: &str,
        pattern: &str,
        range: (f64, f64)
        ...
        let patterns: Vec<Pattern<f64>> = values.into_iter()
            .map(Pattern::pure)
            .collect();
        ...
        if let Some(HybridNode::PatternDsp { voice, .. }) = self.nodes.get_mut(voice_name) {
            voice.params.insert(param_name.to_string(), PatternParam {
                pattern: value_pattern,
            ...
            Err(format!("Voice '{}' not found", voice_name))
        }
    }
    ...
    pub fn query(&self, beat: f64) -> Vec<(String, Vec<String>)> {
        ...
        for (name, node) in &self.nodes {
            match node {
                ...
                HybridNode::PatternDsp { trigger_pattern, .. } => {
