...
impl DslOscHandler {
    pub fn new(sample_rate: f32, block_size: usize) -> Self {
        ...
        let pattern_bridge = Arc::new(RwLock::new(PatternBridge::new(graph.clone(), sample_rate)));
        let modulation_router = Arc::new(RwLock::new(ModulationRouter::new(graph.clone())));
    ...
    fn handle_load_patch(&mut self, msg: &OscMessage) -> Result<(), String> {
        if let Some(OscType::String(patch_text)) = msg.args.first() {
    ...
    fn handle_register_pattern(&mut self, msg: &OscMessage) -> Result<(), String> {
        if let Some(OscType::String(pattern_name)) = msg.args.first() {
            {
                let mut bridge = self.pattern_bridge.write().unwrap();
    ...
    fn handle_add_route(&mut self, msg: &OscMessage) -> Result<(), String> {
        if let Some(OscType::String(route_str)) = msg.args.first() {
            {
                let mut router = self.modulation_router.write().unwrap();
...
mod tests {
    ...
    fn test_set_bus() {
        ...
        let msg = OscMessage {
            ...
            args: vec![OscType::String("~test".to_string()), OscType::Float(0.5)],
        };
