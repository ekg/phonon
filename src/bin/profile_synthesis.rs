...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &size in &buffer_sizes {
        ...
        println!(
            "Buffer size {}: {:.2}ms for {} samples ({:.0} samples/ms, {:.1}x realtime)",
    ...
    if p95 > 11.6 {
        println!(
            "\n⚠️  UNDERRUNS LIKELY: P95 time ({:.2}ms) exceeds budget (11.6ms)",
            p95
        );
    } else {
        println!(
            "\n✓ Performance OK: P95 time ({:.2}ms) within budget (11.6ms)",
            p95
        );
    }
