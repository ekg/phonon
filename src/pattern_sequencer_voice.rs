...
impl PatternSequencer {
    ...
    pub fn process_sample(&mut self) -> f32 {
        ...
        if sample_in_cycle == 0 {
            ...
            eprintln!("Cycle {}: {} active voices", current_cycle, self.voice_manager.active_voice_count());
        }
        ...
        for event in events {
            ...
            if event_start <= cycle_position && event_start > (cycle_position - 1.0/self.samples_per_cycle as f64) {
                let event_key = format!("{}_{}_{:.3}", event.value, current_cycle as i32, event_start);
    ...
    fn trigger_sample(&mut self, sample_name: &str) {
        ...
        if let Some(samples) = bank.get_sample(sample_name) {
            ...
            eprintln!("Warning: Sample '{}' not found", sample_name);
        }
    }
