...
"#;
        ...
        assert!(stats.rms > 0.3 && stats.rms < 0.4, "RMS should be ~0.35 for 0.5 amplitude sine");
...
"#;
        ...
        let stats = renderer.render_to_file(dsl, &output_path)
        ...
        assert!(metadata.len() > expected_size as u64 / 2, "File should have data");
...
"#;
        ...
        assert!(samples[last_idx].abs() < 0.01, "Last sample should be near zero");
        assert!(samples[last_idx - 10].abs() < 0.1, "Late samples should be faded");
        ...
        let mid_section_max = samples[mid_idx-50..mid_idx+50]
            ...
            .map(|x| x.abs())
            .fold(0.0f32, f32::max);
        assert!(mid_section_max > 0.5, "Middle section should have significant amplitude");
    }
}