...
pub fn pattern_to_ascii<T: std::fmt::Display + Clone + Send + Sync + 'static>(
    ...
    // Header
    output.push_str(&format!("Pattern visualization ({} cycles, {} steps/cycle):\n", cycles, resolution));
...
pub fn describe_pattern<T: std::fmt::Display + Clone + Send + Sync + 'static>(
    ...
    let mut output = String::new();
    output.push_str(&format!("Pattern events for {} cycle(s):\n", cycles));
