...
impl Pattern<f64> {
    ...
    pub fn midi(self, channel: u8) -> Pattern<MidiMessage> {
        Pattern::new(move |state: &State| {
            ...
            haps.into_iter().flat_map(|hap| {
                let note = hap.value as u8;
                vec![
                    Hap::new(
                        hap.whole.clone(),
                        TimeSpan::new(hap.part.begin, hap.part.begin),
                        MidiMessage::NoteOn { channel, note, velocity: 100 },
                    ),
                    Hap::new(
                        hap.whole,
        ...
        Pattern::new(move |state: &State| {
            ...
            haps.into_iter().map(|hap| {
                ...
                Hap::new(
                    ...
                    hap.part,
                    vec![
                        MidiMessage::ControlChange { channel, controller: 99, value: param_msb },
                        MidiMessage::ControlChange { channel, controller: 98, value: param_lsb },
                        MidiMessage::ControlChange { channel, controller: 6, value: value_msb },
                        MidiMessage::ControlChange { channel, controller: 38, value: value_lsb },
                    ],
                )
            }).collect()
        })
...
impl MidiSequence {
    ...
    pub fn note(mut self, time: f64, note: u8, velocity: u8, duration: f64, channel: u8) -> Self {
        self.events.push((time, MidiMessage::NoteOn { channel, note, velocity }));
        self.events.push((time + duration, MidiMessage::NoteOff { channel, note, velocity: 0 }));
        self
    }
    
    pub fn cc(mut self, time: f64, controller: u8, value: u8, channel: u8) -> Self {
        self.events.push((time, MidiMessage::ControlChange { channel, controller, value }));
        self
    }
