...
impl<T: Clone + Send + Sync + 'static> Pattern<T> {
    ...
    pub fn with_context<C: Clone + Send + Sync + 'static>(self, context: C) -> Pattern<(T, C)> {
        Pattern::new(move |state: &State| {
            let haps = self.query(state);
            haps.into_iter()
                .map(|hap| Hap::new(hap.whole, hap.part, (hap.value, context.clone())))
                .collect()
        })
    }
...
impl<T: Clone + Send + Sync + 'static> Pattern<T> {
    ...
    pub fn duration_stats(self) -> (f64, f64, f64) {
        ...
        let durations: Vec<f64> = haps.iter().map(|h| h.part.duration().to_float()).collect();
...
impl<T: Clone + Send + Sync + PartialEq + 'static> Pattern<T> {
    ...
    pub fn equivalent_to(self, other: Pattern<T>, cycles: f64) -> bool {
        ...
        for (h1, h2) in haps1.iter().zip(haps2.iter()) {
            if h1.value != h2.value || h1.part.begin != h2.part.begin || h1.part.end != h2.part.end
            {
                return false;
            }
        }
