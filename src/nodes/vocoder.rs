...
mod tests {
    ...
    fn test_vocoder_voice_modulating_synth() {
        ...
        num_bands.process_block(&[], &mut num_bands_buf, sample_rate, &context);
        bandwidth.process_block(&[], &mut bandwidth_buf, sample_rate, &context);
        carrier_osc.process_block(&[carrier_freq_buf.as_slice()], &mut carrier_buf, sample_rate, &context);
        modulator.process_block(&[mod_amp_buf.as_slice()], &mut modulator_buf, sample_rate, &context);
        let inputs = vec![carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()];
        for _ in 0..5 {
            vocoder.process_block(&inputs, &mut output, sample_rate, &context);
        }
        let rms = calculate_rms(&output);
        assert!(rms > 0.01, "Vocoder should produce audible output, got RMS: {}", rms);
        assert!(output.iter().all(|&x| x.is_finite()), "Output should be stable");
    }
    ...
    fn test_vocoder_num_bands_affects_quality() {
        ...
        let mut vocoder_8 = VocoderNode::new(0, 1, 2, 3, sample_rate);
        let mut vocoder_32 = VocoderNode::new(0, 1, 2, 3, sample_rate);
        let num_bands_8_buf = vec![8.0; block_size];
        let num_bands_32_buf = vec![32.0; block_size];
        let carrier_buf = vec![1.0; block_size];
        let modulator_buf = vec![0.5; block_size];
        let bandwidth_buf = vec![1.0; block_size];
        let mut output_8 = vec![0.0; block_size];
        let mut output_32 = vec![0.0; block_size];
        vocoder_8.process_block(&[carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_8_buf.as_slice(), bandwidth_buf.as_slice()], &mut output_8, sample_rate, &context);
        vocoder_32.process_block(&[carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_32_buf.as_slice(), bandwidth_buf.as_slice()], &mut output_32, sample_rate, &context);
        assert_eq!(vocoder_8.num_bands(), 8);
        assert_eq!(vocoder_32.num_bands(), 32);
    }
    ...
    fn test_vocoder_envelope_tracking() {
        ...
        let context = test_context(block_size);
        let carrier_buf = vec![1.0; block_size];
        let num_bands_buf = vec![16.0; block_size];
        let bandwidth_buf = vec![1.0; block_size];
        let modulator_silent = vec![0.0; block_size];
        let modulator_loud = vec![1.0; block_size];
        let mut output_silent = vec![0.0; block_size];
        let mut output_loud = vec![0.0; block_size];
        for _ in 0..10 {
            vocoder.process_block(&[carrier_buf.as_slice(), modulator_silent.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()], &mut output_silent, sample_rate, &context);
        }
        for _ in 0..10 {
            vocoder.process_block(&[carrier_buf.as_slice(), modulator_loud.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()], &mut output_loud, sample_rate, &context);
        }
        let rms_silent = calculate_rms(&output_silent);
        let rms_loud = calculate_rms(&output_loud);
        assert!(rms_loud > rms_silent * 5.0, "Loud modulator should produce more output");
    }
    ...
    fn test_vocoder_stability() {
        ...
        mod_amp.process_block(&[], &mut mod_amp_buf, sample_rate, &context);
        for _ in 0..100 {
            carrier.process_block(&[carrier_freq_buf.as_slice()], &mut carrier_buf, sample_rate, &context);
            modulator.process_block(&[mod_amp_buf.as_slice()], &mut modulator_buf, sample_rate, &context);
            vocoder.process_block(&[carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()], &mut output, sample_rate, &context);
            for (i, &sample) in output.iter().enumerate() {
                assert!(sample.is_finite(), "Sample {} became infinite/NaN", i);
                assert!(sample.abs() < 100.0, "Sample {} has extreme value: {}", i, sample);
            }
        }
    }
    ...
    fn test_vocoder_logarithmic_bands() {
        ...
        let ratio1 = freqs[1] / freqs[0];
        let ratio2 = freqs[2] / freqs[1];
        assert!((ratio1 - ratio2).abs() < 0.01, "Bands should be logarithmically spaced");
    }
    ...
    fn test_vocoder_reset() {
        ...
        let mut vocoder = VocoderNode::new(0, 1, 2, 3, sample_rate);
        vocoder.reset();
        for band in &vocoder.bands {
            assert_eq!(band.envelope_state, 0.0);
        }
    }
    ...
    fn test_vocoder_extreme_parameters() {
        ...
        let carrier_buf = vec![1.0; block_size];
        let modulator_buf = vec![0.5; block_size];
        let num_bands_buf = vec![100.0; block_size];  // Should clamp to 32
        let bandwidth_buf = vec![10.0; block_size];    // Should clamp to 2.0
        let mut output = vec![0.0; block_size];
        vocoder.process_block(&[carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()], &mut output, sample_rate, &context);
    ...
    fn test_vocoder_band_update() {
        ...
        num_bands_buf.fill(24.0);
        vocoder.process_block(&[carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()], &mut output, sample_rate, &context);
        assert_eq!(vocoder.num_bands(), 24);
    }
    ...
    fn test_vocoder_bandwidth_affects_clarity() {
        ...
        let modulator_buf = vec![0.5; block_size];
        let num_bands_buf = vec![16.0; block_size];
        let bandwidth_narrow_buf = vec![0.5; block_size];
        let bandwidth_wide_buf = vec![2.0; block_size];
        let mut output_narrow = vec![0.0; block_size];
        let mut output_wide = vec![0.0; block_size];
        for _ in 0..5 {
            vocoder_narrow.process_block(&[carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_narrow_buf.as_slice()], &mut output_narrow, sample_rate, &context);
            vocoder_wide.process_block(&[carrier_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_wide_buf.as_slice()], &mut output_wide, sample_rate, &context);
        }
        let rms_narrow = calculate_rms(&output_narrow);
        let rms_wide = calculate_rms(&output_wide);
        assert!(rms_narrow > 0.01);
        assert!(rms_wide > 0.01);
    }
    ...
    fn test_vocoder_different_carriers() {
        ...
        let mut carrier_saw = OscillatorNode::new(0, Waveform::Saw);
        let mut carrier_pulse = OscillatorNode::new(0, Waveform::Square);
        let mut vocoder_saw = VocoderNode::new(0, 1, 2, 3, sample_rate);
        let mut vocoder_pulse = VocoderNode::new(0, 1, 2, 3, sample_rate);
        let carrier_freq_buf = vec![220.0; block_size];
        let mut carrier_saw_buf = vec![0.0; block_size];
        let mut carrier_pulse_buf = vec![0.0; block_size];
        let modulator_buf = vec![0.5; block_size];
        let num_bands_buf = vec![16.0; block_size];
        ...
        let mut output_saw = vec![0.0; block_size];
        let mut output_pulse = vec![0.0; block_size];
        carrier_saw.process_block(&[carrier_freq_buf.as_slice()], &mut carrier_saw_buf, sample_rate, &context);
        carrier_pulse.process_block(&[carrier_freq_buf.as_slice()], &mut carrier_pulse_buf, sample_rate, &context);
        for _ in 0..5 {
            vocoder_saw.process_block(&[carrier_saw_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()], &mut output_saw, sample_rate, &context);
            vocoder_pulse.process_block(&[carrier_pulse_buf.as_slice(), modulator_buf.as_slice(), num_bands_buf.as_slice(), bandwidth_buf.as_slice()], &mut output_pulse, sample_rate, &context);
        }
        assert!(calculate_rms(&output_saw) > 0.01);
        assert!(calculate_rms(&output_pulse) > 0.01);
    }
