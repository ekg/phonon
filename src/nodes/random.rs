/// Random value generator (white noise with amplitude control)
///
...
/// # Example
/// ```ignore
/// // Generate random values at 0.5 amplitude
/// let amplitude = ConstantNode::new(0.5);  // NodeId 0
/// let random = RandomNode::new(0);          // NodeId 1
...
impl RandomNode {
    ...
    /// Create a new random node with a specific seed (for testing)
    ///
    /// # Arguments
    /// * `amplitude_input` - NodeId of the amplitude control signal
...
impl AudioNode for RandomNode {
    fn process_block(
        ...
        debug_assert!(
            inputs.len() >= 1,
            "RandomNode requires 1 input (amplitude), got {}",
            inputs.len()
        );
        ...
        // Generate random values: random values in [-1, 1] scaled by amplitude
        for i in 0..output.len() {
            // Generate random value in [-1.0, 1.0]
            let random_value = self.rng.gen::<f32>() * 2.0 - 1.0;
...
mod tests {
    ...
    fn test_random_generates_different_values() {
        ...
        assert!(!all_same, "Random output should vary between samples");
    }

    #[test]
    fn test_random_within_amplitude_range() {
    ...
    #[test]
    fn test_random_average_near_zero() {
    ...
    fn test_random_seed_reproducibility() {
        ...
        let mut random1 = RandomNode::new_with_seed(0, 12345);
        let mut random2 = RandomNode::new_with_seed(0, 12345);
    ...
    fn test_random_dependencies() {
        let random = RandomNode::new(7);
        let deps = random.input_nodes();
    ...
    #[test]
    fn test_random_different_seeds_different_output() {
        // Different seeds should produce different output
        let mut random1 = RandomNode::new_with_seed(0, 111);
        let mut random2 = RandomNode::new_with_seed(0, 222);
    ...
    #[test]
    fn test_random_with_varying_amplitude() {
