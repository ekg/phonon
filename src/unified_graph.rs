...
fn synthesize_bus_buffer_parallel(
    nodes: Vec<Option<Arc<SignalNode>>>,
    bus_node_id: NodeId,
    duration_samples: usize,
    sample_rate: f32,
) -> Vec<f32> {
    ...
    for node_opt in nodes.iter() {
        if let Some(node_arc) = node_opt {
            if let SignalNode::Oscillator { phase, .. } = &**node_arc {
...
fn eval_node_isolated(nodes: &mut Vec<Option<Arc<SignalNode>>>, node_id: &NodeId, sample_rate: f32) -> f32 {
    ...
    match node {
        ...
        SignalNode::Biquad {
            ...
            if let Some(Some(node_arc)) = nodes.get_mut(node_id.0) {
...
impl UnifiedSignalGraph {
    ...
    pub fn write_tap_files(&self) -> Vec<String> {
        ...
        for node_option in &self.nodes {
            if let Some(node_rc) = node_option {
                if let SignalNode::Tap { state, .. } = &**node_rc {
    ...
    pub fn get_node(&self, node_id: NodeId) -> Option<&Arc<SignalNode>> {
    ...
    fn eval_note_signal_at_time(&mut self, signal: &Signal, cycle_pos: f64) -> f32 {
        match signal {
            Signal::Node(id) => {
                if let Some(Some(node_rc)) = self.nodes.get(id.0) {
                    if let SignalNode::Pattern { pattern, .. } = &**node_rc {
                ...
                let nodes_copy: Vec<Option<Arc<SignalNode>>> = self.nodes
                    ...
                    .map(|opt_arc| {
                        opt_arc.as_ref().map(|arc_node| {
                            // Deep clone the SignalNode and wrap in new Arc
                            Arc::new((**arc_node).clone())
                        })
                    })
                    .collect();
    ...
    fn eval_node(&mut self, node_id: &NodeId) -> f32 {
        ...
        // PERFORMANCE FIX: Use Arc::clone for cheap reference counting
        // Arc::clone just increments a counter (~5ns vs ~500ns for deep clone)
        ...
        let value = match &*node {
            ...
            SignalNode::Blip { frequency, phase } => {
                ...
                let phase_inc = freq / self.sample_rate;
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::Blip { phase, .. } = &**node_rc {
                        let mut p = phase.borrow_mut();
                        *p += phase_inc;
                        if *p >= 1.0 {
                            *p -= 1.0;
            ...
            SignalNode::VCO {
                ...
                // Update phase for next sample
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::VCO { phase, .. } = &**node_rc {
                        let mut p = phase.borrow_mut();
                        *p += phase_inc;
                        if *p >= 1.0 {
                            *p -= 1.0;
            ...
            SignalNode::Granular {
                ...
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::Granular { state: s, .. } = &**node_rc {
            ...
            SignalNode::Formant {
                ...
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::Formant { state: s, .. } = &**node_rc {
            ...
            SignalNode::Vowel {
                ...
                // Get mutable state and process
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::Vowel { state: s, .. } = &**node_rc {
            ...
            SignalNode::Vocoder {
                ...
                // Get mutable state and process
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::Vocoder { state: s, .. } = &**node_rc {
            ...
            SignalNode::PitchShift {
                ...
                // Get mutable state and process
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::PitchShift { state: s, .. } = &**node_rc {
            ...
            SignalNode::SVF {
                ...
                // Update state
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::SVF { state: s, .. } = &**node_rc {
                        s.low = low;
                        s.band = band;
                    }
                }
            ...
            SignalNode::Biquad {
                ...
                // Update state
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::Biquad { state: s, .. } = &**node_rc {
                        s.x2 = x1;
                        s.x1 = input_val;
                        s.y2 = y1;
                        s.y1 = final_output;
            ...
            SignalNode::Resonz {
                ...
                // Update state
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::Resonz { state: s, .. } = &**node_rc {
                        s.x2 = x1;
                        s.x1 = input_val;
                        s.y2 = y1;
                        s.y1 = final_output;
            ...
            SignalNode::RLPF {
                ...
                // Update state
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::RLPF { state: s, .. } = &**node_rc {
                        s.x2 = x1;
                        s.x1 = input_val;
                        s.y2 = y1;
                        s.y1 = final_output;
            ...
            SignalNode::RHPF {
                ...
                // Update state
                if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                    if let SignalNode::RHPF { state: s, .. } = &**node_rc {
                        s.x2 = x1;
                        s.x1 = input_val;
                        s.y2 = y1;
                        s.y1 = final_output;
            ...
            SignalNode::Reverb {
                ...
                for i in 0..8 {
                    let buf_len = state.borrow().comb_buffers[i].len();
                    let read_idx = state.borrow_mut().comb_indices[i];
                    let delayed = state.borrow().comb_buffers[i][read_idx];

                    // Lowpass filter for damping
                    let filtered = state.borrow_mut().comb_filter_stores[i] * damp + delayed * (1.0 - damp);
                    ...
                    // Update state
                    if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                        if let SignalNode::Reverb { state: s, .. } = &**node_rc {
                            let mut state = s.borrow_mut();
                            state.comb_buffers[i][read_idx] = to_write;
                            state.comb_indices[i] = (read_idx + 1) % buf_len;
                            state.comb_filter_stores[i] = filtered;
                        }
                    }
                }
                ...
                for i in 0..4 {
                    let buf_len = state.borrow().allpass_buffers[i].len();
                    let read_idx = state.borrow_mut().allpass_indices[i];
                    let delayed = state.borrow().allpass_buffers[i][read_idx];
                    ...
                    if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                        if let SignalNode::Reverb { state: s, .. } = &**node_rc {
                            let mut state = s.borrow_mut();
                            state.allpass_buffers[i][read_idx] = to_write;
                            state.allpass_indices[i] = (read_idx + 1) % buf_len;
                        }
                    }
                }
            ...
            SignalNode::Convolution { input, state } => {
                ...
                let output = if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::Convolution { state: s, .. } = &**node_rc {
                        s.borrow_mut().process(input_val)
                    } else {
                        input_val
                    }
                } else {
            ...
            SignalNode::SpectralFreeze {
                ...
                let output = if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::SpectralFreeze { state: s, .. } = &**node_rc {
                        s.borrow_mut().process(input_val, trigger_val)
                    } else {
                        input_val
                    }
                } else {
            ...
            SignalNode::BitCrush {
                ...
                let phase = *state.borrow().phase.borrow() + rate_reduction;
                let mut output = *state.borrow_mut().last_sample.borrow();
                ...
                if phase >= 1.0 {
                    ...
                    if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                        if let SignalNode::BitCrush { state: s, .. } = &**node_rc {
                            let mut state = s.borrow_mut();
                            *state.phase.borrow_mut() = phase - phase.floor();
                            *state.last_sample.borrow_mut() = quantized;
                        }
                    }
                } else if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::BitCrush { state: s, .. } = &**node_rc {
                        let mut state = s.borrow_mut();
                        *state.phase.borrow_mut() = phase;
                    }
                }
            ...
            SignalNode::Chorus {
                ...
                let lfo_phase = state.borrow_mut().lfo_phase;
                ...
                let buf_len = state.borrow_mut().delay_buffer.len();
                let read_pos =
                    (state.borrow_mut().write_idx as f32 + buf_len as f32 - delay_samples) % buf_len as f32;
                let read_idx = read_pos.floor() as usize;
                let frac = read_pos - read_pos.floor();

                let sample1 = state.borrow().delay_buffer[read_idx % buf_len];
                let sample2 = state.borrow().delay_buffer[(read_idx + 1) % buf_len];
                let delayed = sample1 + (sample2 - sample1) * frac;

                // Update state
                if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::Chorus { state: s, .. } = &**node_rc {
                        let mut state = s.borrow_mut();
                        state.delay_buffer[state.write_idx] = input_val;
                        state.write_idx = (state.write_idx + 1) % buf_len;
                        state.lfo_phase = (lfo_phase + lfo_rate / self.sample_rate) % 1.0;
                    }
                }
            ...
            SignalNode::Flanger {
                ...
                if mod_depth < 0.01 {
                    ...
                    if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                        if let SignalNode::Flanger { state: s, .. } = &**node_rc {
                            let mut state_mut = s.borrow_mut();
                            state_mut.lfo_phase = (state_mut.lfo_phase + lfo_rate / self.sample_rate) % 1.0;
                        }
                    }
                    return input_val;
                }
                ...
                let wet = delayed + state.borrow_mut().feedback_sample * feedback_amt;
            ...
            SignalNode::Compressor {
                ...
                if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::Compressor { state: s, .. } = &**node_rc {
                        let mut state = s.borrow_mut();
            ...
            SignalNode::Tremolo {
                ...
                if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::Tremolo { phase: p, .. } = &**node_rc {
                        // Advance phase
            ...
            SignalNode::RingMod { input, freq, phase } => {
                ...
                if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::RingMod { phase: p, .. } = &**node_rc {
                        let mut phase_val = p.borrow_mut();
                        *phase_val += carrier_freq * 2.0 * std::f32::consts::PI / self.sample_rate;
                        if *phase_val >= 2.0 * std::f32::consts::PI {
                            *phase_val -= 2.0 * std::f32::consts::PI;
                        }
                        let carrier = phase_val.sin();
            ...
            SignalNode::Pattern {
                ...
                if let Some(event) = events.first() {
                    ...
                    if s.trim() == "~" {
                        ...
                        if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                            if let SignalNode::Pattern { last_value: lv, .. } = &**node_rc {
                                *lv.borrow_mut() = 0.0;
                            }
                        }
            ...
            SignalNode::CycleTrigger {
                ...
                if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::CycleTrigger { last_cycle: lc, .. } = &**node_rc {
            ...
            SignalNode::HighPass {
                ...
                let (mut low, mut band, mut high) =
                    if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                        if let SignalNode::HighPass { state, .. } = &**node_rc {
                            let s = state.borrow();
                            (s.y1, s.x1, s.y2)
                        } else {
                            (0.0, 0.0, 0.0)
            ...
            SignalNode::BandPass {
                ...
                let (mut low, mut band, mut high) =
                    if let Some(Some(node_rc)) = self.nodes.get(node_id.0) {
                        if let SignalNode::BandPass { state, .. } = &**node_rc {
                            let s = state.borrow();
                            (s.y1, s.x1, s.y2)
                        } else {
                            (0.0, 0.0, 0.0)
            ...
            SignalNode::Envelope {
                ...
                let mut output_level = state.borrow().level;

                // Check for trigger
                {
                    let phase = state.borrow().phase.borrow().clone();
                    if trig > 0.5 && matches!(phase, EnvPhase::Idle | EnvPhase::Release) {
                        *state.borrow().phase.borrow_mut() = EnvPhase::Attack;
                        ...
                        )
                    {
                        ...
                        let current_level = state.borrow().level;
                        state.borrow_mut().release_start_level = current_level;
                ...
                state.borrow_mut().time_in_phase += dt;

                match *state.borrow().phase.borrow() {
                    EnvPhase::Attack => {
                        if attack_val > 0.0 {
                            state.borrow_mut().level = state.borrow().time_in_phase / attack_val;
                    ...
                    EnvPhase::Decay => {
                        if decay_val > 0.0 {
                            state.borrow_mut().level =
                                1.0 - (1.0 - sustain_val) * (state.borrow().time_in_phase / decay_val);
                            if state.borrow().level <= sustain_val {
                    ...
                    EnvPhase::Release => {
                        if release_val > 0.0 {
                            ...
                            let progress = (state.borrow().time_in_phase / release_val).min(1.0);
                            state.borrow_mut().level = state.borrow().release_start_level * (1.0 - progress);

                            if progress >= 1.0 {
                                state.borrow_mut().level = 0.0;
                                *state.borrow().phase.borrow_mut() = EnvPhase::Idle;
                            }
                        } else {
                            state.borrow_mut().level = 0.0;
                            *state.borrow().phase.borrow_mut() = EnvPhase::Idle;
            ...
            SignalNode::EnvelopePattern {
                ...
                {
                    let phase = state.borrow().phase.borrow();
            ...
            SignalNode::TapeDelay {
                ...
                let buffer_len = state.borrow_mut().buffer.len();
                ...
                let wow = (state.borrow_mut().wow_phase * std::f32::consts::TAU).sin() * wow_d * 0.001;
                let flutter = (state.borrow_mut().flutter_phase * std::f32::consts::TAU).sin() * flutter_d * 0.0001;
                ...
                let read_pos_f = (state.borrow_mut().write_idx as f32) - delay_samples;
                ...
                let delayed = state.borrow().buffer[read_idx] * (1.0 - frac) + state.borrow().buffer[next_idx] * frac;
                ...
                let filtered = state.borrow_mut().lpf_state * cutoff_coef + saturated * (1.0 - cutoff_coef);
                ...
                // Update state
                if let Some(Some(node_rc)) = self.nodes.get_mut(node_id.0) {
                    if let SignalNode::TapeDelay { state: s, .. } = &**node_rc {
                        let mut state_mut = s.borrow_mut();
                        state_mut.buffer[state_mut.write_idx] = to_write;
                        state_mut.write_idx = (state_mut.write_idx + 1) % buffer_len;
                        state_mut.wow_phase = (state_mut.wow_phase + wow_phase_inc) % 1.0;
                        state_mut.flutter_phase = (state_mut.flutter_phase + flutter_phase_inc) % 1.0;
                        state_mut.lpf_state = filtered;
                    }
                }
