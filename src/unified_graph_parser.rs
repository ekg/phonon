...
use crate::unified_graph::{UnifiedSignalGraph, SignalNode, Signal, SignalExpr, Waveform};
use crate::mini_notation_v3::parse_mini_notation;
use nom::{
    IResult,
    branch::alt,
    bytes::complete::{tag, take_until},
    character::complete::{char, digit1, multispace0, multispace1, alpha1, alphanumeric1},
...
fn bus_ref(input: &str) -> IResult<&str, DslExpression> {
    map(
        ...
        |name: &str| DslExpression::BusRef(name.to_string())
    )(input)
}
...
fn oscillator(input: &str) -> IResult<&str, DslExpression> {
    map(
        tuple((waveform, function_args)),
        |(wf, args)| {
            let freq = args.get(0).cloned().unwrap_or(DslExpression::Value(440.0));
...
fn filter(input: &str) -> IResult<&str, DslExpression> {
    let lpf = map(
        preceded(tag("lpf"), function_args),
        |args| DslExpression::Filter {
            filter_type: FilterType::LowPass,
            input: Box::new(args.get(0).cloned().unwrap_or(DslExpression::Value(0.0))),
    ...
    let hpf = map(
        preceded(tag("hpf"), function_args),
        |args| DslExpression::Filter {
...
fn delay(input: &str) -> IResult<&str, DslExpression> {
    map(
        preceded(tag("delay"), function_args),
        |args| DslExpression::Delay {
            input: Box::new(args.get(0).cloned().unwrap_or(DslExpression::Value(0.0))),
...
fn rms_analyzer(input: &str) -> IResult<&str, DslExpression> {
    map(
        preceded(tag("rms"), function_args),
        |args| DslExpression::Analysis {
            analysis_type: AnalysisType::RMS,
            input: Box::new(args.get(0).cloned().unwrap_or(DslExpression::Value(0.0))),
            params: vec![
                args.get(1)
                    .and_then(|e| if let DslExpression::Value(v) = e { Some(*v) } else { None })
                    .unwrap_or(0.01)
            ],
        }
    )(input)
...
fn when_expr(input: &str) -> IResult<&str, DslExpression> {
    map(
        preceded(tag("when"), function_args),
        |args| DslExpression::When {
            input: Box::new(args.get(0).cloned().unwrap_or(DslExpression::Value(0.0))),
...
fn chain(input: &str) -> IResult<&str, DslExpression> {
    ...
    let (input, chains) = many0(preceded(
        ws(tag(">>")),
        arithmetic
    ))(input)?;

    let expr = chains.into_iter().fold(first, |acc, right| {
        DslExpression::Chain {
            left: Box::new(acc),
            right: Box::new(right),
        }
...
impl DslCompiler {
    ...
    fn compile_expression(&mut self, expr: DslExpression) -> crate::unified_graph::NodeId {
        match expr {
            ...
            DslExpression::Value(v) => {
                self.graph.add_node(SignalNode::Constant { value: v })
            }
            ...
            DslExpression::Filter { filter_type, input, cutoff, q } => {
                ...
                match filter_type {
                    ...
                    FilterType::HighPass => {
                        self.graph.add_node(SignalNode::HighPass {
                            input: input_signal,
                            cutoff: cutoff_signal,
                            q: q_signal,
                            state: Default::default(),
            ...
            DslExpression::BinaryOp { op, left, right } => {
                ...
                match op {
                    BinaryOperator::Add => {
                        self.graph.add_node(SignalNode::Add {
                            a: left_signal,
                            b: right_signal,
                        })
                    }
