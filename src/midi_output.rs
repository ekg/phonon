...
impl MidiOutputHandler {
    ...
    pub fn connect_to_port(&mut self, port: MidiOutputPort) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn play_pattern<T: Clone + Send + Sync + 'static>(&self, 
        pattern: &Pattern<T>, 
        tempo_bpm: f32,
        duration_beats: f32,
        ...
        while current_beat < duration_beats {
            ...
            for event in events {
                if let Some(midi_msg) = midi_converter(&event.value) {
                    ...
                    // Schedule note-off
                    if let MidiMessage::NoteOn { channel, note, velocity } = midi_msg {
                        let duration = (event.part.end - event.part.begin).to_float();
                        let off_time = now + Duration::from_secs_f32(duration as f32 * beat_duration);
