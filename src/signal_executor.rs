...
use crate::signal_graph::{
    AnalysisType, BusId, Node, NodeId, ProcessorType, SignalGraph, SourceType,
};
use fundsp::hacker::*;
    ...
    pub fn create_processor(&mut self, node: &Node) -> Result<(), String> {
        let processor: Box<dyn AudioUnit> = match node {
            Node::Source { id: _, source_type } => {
                match source_type {
                    SourceType::Sine { freq } => Box::new(sine_hz(*freq as f32)),
                    SourceType::Saw { freq } => Box::new(saw_hz(*freq as f32)),
                    SourceType::Square { freq } => Box::new(square_hz(*freq as f32)),
                    SourceType::Triangle { freq } => Box::new(triangle_hz(*freq as f32)),
                    SourceType::Noise => Box::new(white()),
                    SourceType::Sample { .. } => {
            ...
            Node::Processor {
                ...
                match processor_type {
                    ...
                    ProcessorType::Gain { amount } => Box::new(mul(*amount)),
                }
            }
            Node::Analysis { .. } => {
            ...
            Node::Bus { .. } => Box::new(zero()),
            Node::Output { .. } => Box::new(pass()),
        };
