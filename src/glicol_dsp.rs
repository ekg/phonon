        ...
        for (i, node) in self.nodes.iter().enumerate() {
            ...
            let graph_node = match node {
                ...
                DspNode::Add { value } => Node::Processor {
                    ...
                    processor_type: ProcessorType::Gain {
                        amount: (1.0 + *value),
                    },
                },
                ...
                DspNode::Reverb {
                    ...
                    id: node_id.clone(),
                    processor_type: ProcessorType::Reverb { mix: *mix as f64 },
                },
                DspNode::Gain { amount } => Node::Processor {
                    id: node_id.clone(),
                    processor_type: ProcessorType::Gain { amount: *amount },
                },
                DspNode::Seq { pattern } => Node::Pattern {
                    id: node_id.clone(),
...
impl Default for DspEnvironment {
    fn default() -> Self {
        Self::new()
    }
}
...
impl DspEnvironment {
    ...
    pub fn build_complete_graph(&self, sample_rate: f32) -> Result<SignalGraph, String> {
        ...
        for (name, chain) in &self.ref_chains {
            ...
            let chain_graph = chain.clone().build_graph(sample_rate)?;
        ...
        if let Some(output) = &self.output_chain {
            let output_graph = output.clone().build_graph(sample_rate)?;
...
pub mod dsp {
    ...
    pub fn seq(pattern: &str) -> DspChain {
        DspChain::from_node(DspNode::Seq {
