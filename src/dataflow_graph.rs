...
impl DataflowGraph {
    ...
    ///
    /// This sends a trigger to source nodes and receives the processed output.
    /// The dataflow graph processes the block continuously in the background.
    ///
    /// # Arguments
    ...
    pub fn process_block(&mut self, output: &mut [f32]) -> Result<(), String> {
...
mod tests {
    ...
    fn test_dataflow_graph_single_block() {
        ...
        let mut graph = DataflowGraph::new(nodes, 0, context).unwrap();
        ...
        let result = graph.process_block(&mut output);
    ...
    fn test_dataflow_graph_pipeline() {
        ...
        let mut graph = DataflowGraph::new(nodes, 1, context).unwrap();
        ...
        // Process one block
        let mut output = vec![0.0; 512];
        graph.process_block(&mut output).unwrap();
