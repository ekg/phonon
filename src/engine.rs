...
impl AudioEngine {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        info!(
            "Sample format: {:?}, Rate: {} Hz, Channels: {}",
    ...
    pub fn schedule_sample(&self, sample_id: &str, time: f64, speed: f32, gain: f32) {
        ...
        scheduler
            .events
            .make_contiguous()
            .sort_by_key(|e| e.trigger_frame);

        debug!(
            "Scheduled {} at frame {} ({}s)",
            sample_id, trigger_frame, time
        );
    }
    ...
    fn ensure_sample_loaded(&self, sample_id: &str) {
        ...
        for (name, path) in sample_paths {
            if name == base_name && index == 0 {
                // Only use hardcoded path for index 0
        ...
        if sample_dir.exists() && sample_dir.is_dir() {
            ...
            if let Ok(entries) = std::fs::read_dir(&sample_dir) {
                ...
                if let Some(path) = wav_files.get(index) {
                    ...
                    warn!(
                        "Sample index {} out of range for {} (has {} files)",
                        index,
                        base_name,
                        wav_files.len()
                    );
