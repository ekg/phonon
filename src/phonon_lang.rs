...
impl PhononEnv {
    ...
    fn parse_expression(&mut self, expr: &str) -> Result<PhononValue, String> {
        ...
        if expr.contains(">>") {
            ...
            if parts[0].starts_with("s ") || parts[0].starts_with("s\"") ||
               parts[0].starts_with("stack") || parts[0].starts_with("cat") {
    ...
    fn parse_pattern_source(&mut self, expr: &str) -> Result<PhononValue, String> {
        ...
        Ok(PhononValue::PatternString(Pattern::from_string(pattern_str)))
    }
    ...
    fn apply_pattern_transformation(&self, pattern: Pattern<String>, transform: &str) -> Result<Pattern<String>, String> {
                        ...
                        if inner.starts_with('(') && inner.ends_with(')') {
                            ...
                            match inner_transform {
                                ...
                                transform if transform.starts_with("fast ") => {
                                    let factor: f64 = transform[5..].parse()
                                ...
                                _ => Err(format!("Unsupported transformation in every: {}", inner_transform))
                            }
                        } else {
            ...
            match transform {
                ...
                _ => {
                    ...
                    if transform.ends_with("()") {
                        let op = &transform[..transform.len()-2];
                        match op {
                            "rev" => Ok(pattern.rev()),
                            "palindrome" => Ok(pattern.palindrome()),
                            "degrade" => Ok(pattern.degrade()),
