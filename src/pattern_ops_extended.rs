...
impl<T: Clone + Send + Sync + 'static> Pattern<T> {
    ...
    pub fn within(self, begin: f64, end: f64, f: impl Fn(Pattern<T>) -> Pattern<T> + Send + Sync + 'static) -> Self {
        let b = Fraction::from_float(begin);
        let e = Fraction::from_float(end);
        let f = Arc::new(f);
        Pattern::new(move |state: &State| {
    ...
    pub fn shuffle(self, amount: f64) -> Self {
        Pattern::new(move |state: &State| {
            ...
            haps.into_iter().map(|mut hap| {
                ...
                hap.part = TimeSpan::new(hap.part.begin + shift_frac, hap.part.end + shift_frac);
                if let Some(whole) = hap.whole.as_mut() {
    ...
    pub fn mask(self, mask_pattern: Pattern<bool>) -> Self {
        Pattern::new(move |state: &State| {
            ...
            haps.into_iter().filter(|hap| {
                mask_haps.iter().any(|mask_hap| {
                    mask_hap.value && mask_hap.part.begin <= hap.part.begin
                })
            }).collect()
        })
    }
    ...
    pub fn chunk_with(self, n: usize, f: impl Fn(usize, Pattern<T>) -> Pattern<T> + Send + Sync + 'static) -> Self {
        let f = Arc::new(f);
        Pattern::new(move |state: &State| {
...
impl<T: Clone + Send + Sync + Debug + 'static> Pattern<T> {
    ...
    pub fn map<U: Clone + Send + Sync + 'static>(
        ...
        Pattern::new(move |state: &State| {
            let haps = self.query(state);
            haps.into_iter().map(|hap| {
                Hap::new(hap.whole, hap.part, f(hap.value))
            }).collect()
        })
    }
...
mod tests {
    ...
    fn test_filter() {
        ...
        let filtered = p.map(|s| s.parse::<i32>().unwrap_or(0))
            .filter(|&n| n > 2);
        let state = State {
            span: TimeSpan::new(Fraction::new(0, 1), Fraction::new(1, 1)),
            controls: HashMap::new(),
